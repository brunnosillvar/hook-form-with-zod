 ...
 * [x] Field Arrays (campos que podem ser inúmeros de acordo com a escolha do usuário)
...
const createUserFormSchema = z.object({
  avatar: z.instanceof(FileList)
    ...
    .refine(file => file!.size <= 5 * 1024 * 1024, 'O arquivo precisa ter no máximo 5Mbytes'),
  name: z.string()
  ...
  email: z.string()
    ...
    }, ' O e-mail precisa ser da Hotmail'),
  password: z.string()
...
export function App() {
  ...
  async function createUser(data: CreateUserFormData) {
    await supabase.storage.from('hook-form-with-zod').upload(data.avatar.name, data.avatar)
            ...
            return (
              <div key={field.id} className='flex gap-2'>
                ...
                <div className='flex-1 flex flex-col gap-1'>
                  <input 
                    type='number'             
                    className='w-16 border border-zinc-800 bg-zinc-900 shadow-sm rounded h-10 px-3 text-white'
